...
3. Dans *Environment Variables*, ajouter `VITE_GOOGLE_CLIENT_ID` (voir `.env.example`), puis déployer.
...
Tout est stocké **localement sur l'appareil** (IndexedDB, via Dexie),
pas sur un serveur. Avantages : ça marche hors connexion, c'est rapide,
et il n'y a pas de compte à créer. Limite actuelle : les données ne se
synchronisent pas automatiquement entre deux appareils (ex. téléphone +
tablette au salon) — chacun a ses propres données.
...
- **Lien agenda** : ta cliente utilise l'app *Calendars* sur iPhone.
  ...
  la cliente.
- **Synchronisation multi-appareils** : si besoin de synchroniser entre
  plusieurs appareils, il faudra ajouter un backend léger (ex.
  Supabase/Firebase) — la couche `db/db.ts` est faite pour pouvoir
  brancher ça sans réécrire les écrans.
